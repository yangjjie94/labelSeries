...
LabelSeries是一个基于Python语言开发的项目，它是个标注软件，可以标注连续的图片，并支持多种形状的标注，包括框选框、线段、椭圆和多边形。label series of pictures. It supports label of bounding boxes, lines, ellipses and polygons.

这个软件本身设计来用于用于从高速摄像得到的分散两相流的图片中识别液滴，后将使用标注数据

我们将使用得到的数据来包括自动检测液滴轮廓、追踪液滴变形导致的破碎聚并现象并生成分析报告，包括液滴的尺寸分布、破碎后的子液滴分布、聚并的分布。该项目正在开发中，版本v0.1.0，地址请点击[Lasso](https://github.com/yangjjie94/lasso/blob/master/README.md)
